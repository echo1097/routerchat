...
RouterChat runs on your computer. It contains no telemetry, no analytics, and no periodic update check. It does not send your prompts, chats, settings, or API key anywhere except to OpenRouter, as described below. The connections it makes are described here; links you choose to open take you to the linked websites under their own terms and privacy policies.

* **OpenRouter.** RouterChat uses the OpenRouter API to validate your key, list models, send model requests, and retrieve usage information. Chat, writing, brainstorming, and lorebook features send the relevant prompts, conversation history, story text, and lorebook context to OpenRouter and whichever upstream provider serves the model used for that feature, under those parties' terms and privacy policies. Automatic chat naming and optional automatic lorebook updates make additional model requests that can incur charges. Attached images and PDFs are sent with the relevant requests; text and code attachments are included as text and may be truncated. If you switch on web search for a chat, each message you send in it also causes OpenRouter to run a web search on your behalf before the model answers, which sends your search terms to the search provider OpenRouter uses for that model, and OpenRouter charges you for each search in addition to the tokens. Web search is off unless you turn it on.
* **Voice input (OpenRouter).** When you submit a microphone recording for transcription, RouterChat sends the audio to OpenRouter and the selected transcription provider. Transcription can incur charges even if you do not send the resulting text as a prompt. Voice input is unavailable while Privacy mode or Zero Data Retention is enabled because the transcription endpoint does not guarantee those routing settings.
...
* **Website icons.** When RouterChat displays source links or links formatted as citations, it may fetch website icons, including when web search is off or older content is reopened. These requests go from your computer to the website and may follow redirects or fetch an icon hosted on another domain. They reveal your IP address and carry no browser cookies, referrer, prompts, chats, settings, or API keys, but cookies received during an icon fetch may be sent in subsequent requests within that fetch. Finding an icon can require several requests. Results are cached locally for about thirty days before they may be fetched again. RouterChat does not use a dedicated third-party icon lookup service, and requests to local or private network addresses are refused.
* **GitHub.** The installer and updater download the RouterChat package, the `uv` tool used to build the private environment, and the private Python runtime itself, all from GitHub releases. Update tooling also checks the latest release and fetches installer and updater scripts and updater checksums from the RouterChat distribution site. This happens only when you run it. Separately, the app automatically opens the changelog the first time you use an installation or version whose changelog has not been marked as seen. Each time the changelog opens, automatically or manually, it fetches the latest published release notes from GitHub, which may be for a newer version than the one installed.
* **Python package sources.** Installation and updates download the third-party packages pinned in `requirements.lock` from the public Python package index, verifying each against the hashes in that file.

Updates are never installed automatically. Apart from the changelog requests described above, RouterChat does not check for new versions on its own. The requests described here can happen automatically as part of using a feature, including displaying citations, naming a chat, or updating a lorebook.
...
* **Costs and usage.** All charges, rate limits, quotas, outages, suspensions, and account actions imposed by OpenRouter or any other provider, including but not limited to transcription, automatic chat naming, lorebook operations, and the per-search charges that web search adds to every message you send while it is on.
...
* **Your data.** Local chats, stories, lorebooks, settings, attachments, and backups. Update backups include the database and local credentials, and the uninstaller offers to save the database before removal. Neither includes the separate attachment files. Both are conveniences offered on a best effort basis, not a backup or recovery service, and neither is guaranteed to succeed. With RouterChat stopped, keep your own copy of `routerchat.sqlite3` and the accompanying `attachments` directory before updating or uninstalling. Keep any credential backup private.
* **Temporary chats.** Temporary chats are written to the local database while in use, and attachments are stored locally. RouterChat attempts to delete them when you leave the chat or page and cleans up remaining temporary chats on backend startup. Temporary mode does not mean that content is never written to disk, and it does not change what is sent to OpenRouter or the providers' retention policies.
