...
* **OpenRouter.** While you use RouterChat, the only connection carrying your prompts is to the OpenRouter API, to list models and to send the requests you initiate. Your prompts and their responses travel to OpenRouter and to whichever upstream provider serves the model you selected, and are handled under those parties' terms and privacy policies. Any file you attach to a message is uploaded to OpenRouter with that request. If you switch on web search for a chat, each message you send in it also causes OpenRouter to run a web search on your behalf before the model answers, which sends your search terms to the search provider OpenRouter uses for that model, and OpenRouter charges you for each search in addition to the tokens. Web search is off unless you turn it on.
...
* **Websites a response cites.** When web search is on and a model response cites a source, RouterChat asks that website for its icon so the source can be shown alongside it. The request goes from your computer directly to that website, carries no cookies, no referrer, and none of your prompts, chats, settings, or keys, and tells it your IP address and that a request was made. It happens at most once per website, after which the icon is stored on your computer and reused for thirty days. No third-party icon service is involved, and requests to local or private network addresses are refused. A website is contacted only if a reply you received cites it.
...
* **Costs and usage.** All charges, rate limits, quotas, outages, suspensions, and account actions imposed by OpenRouter or any other provider, including the per-search charges that web search adds to every message you send while it is on.
